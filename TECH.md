...
- **`internal/caddynode`**: порт `src/caddy/install_node.sh` (180 строк, реально node-only, без путаницы в имени). Написан, собирается, **не протестирован** (нет сети до GitHub-архива шаблонов/certbot в моменте написания, плюс среда только что восстанавливалась после сброса). Проверить перед пушем: `go build`, `go vet`, `gofmt -l`, ручная сверка docker-compose.yml/Caddyfile против оригинала построчно. Оригинал был доступен в `/mnt/user-data/uploads/remnawave-reverse-proxy-3_0_0.zip` при написании, но именно в момент, когда писался этот файл, доступа к архиву не было. Предыдущая сверка делалась по live-просмотру файла раньше в той же сессии, не автоматизированным diff'ом.
