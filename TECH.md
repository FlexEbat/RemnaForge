...
- Go 1.22+, весь код на stdlib, кроме `golang.org/x/net/publicsuffix` (`internal/domain`).
- Внешние зависимости в `go.mod` не запрещены. Добавляйте их, когда это реально лучше stdlib. См. `README.md#установка`.
- Песочница разработки не имеет доступа ни к `proxy.golang.org`, ни к `golang.org` напрямую. Домен `golang.org` заблокирован целиком, поэтому `go get golang.org/x/net` не работает даже с `GOPROXY=direct`: Go не может определить настоящий VCS-адрес модуля через redirect на `golang.org`. `github.com` доступен, и `github.com/golang/net` зеркалирует тот же код. `go.mod` тянет `golang.org/x/net` через `replace golang.org/x/net vX.Y.Z => github.com/golang/net vX.Y.Z`. Команды для обновления версии зависимости: `go get github.com/golang/net@vX.Y.Z`, затем `go mod edit -replace=golang.org/x/net@vX.Y.Z=github.com/golang/net@vX.Y.Z`, затем `GOPROXY=direct GOSUMDB=off go mod download golang.org/x/net`. `replace` работает одинаково при полном доступе к сети, поэтому CI и другие разработчики не замечают разницы.
...
5. Если менялось поведение, видимое пользователю (новый пункт меню, новый файл конфига, новое сообщение), обновить `README.md` (разделы «Возможности», «Требования», «Использование»).
...
### 1.1.6: publicsuffix стал настоящей go.mod-зависимостью, README переписан
`internal/publicsuffix` (вендоренная копия `golang.org/x/net/publicsuffix`) удалён. `internal/domain` теперь импортирует `golang.org/x/net/publicsuffix` напрямую как обычную зависимость `go.mod`. Песочница блокирует сам домен `golang.org`, поэтому обычный `go get` не резолвит vanity-import-путь: `go.mod` содержит `replace golang.org/x/net => github.com/golang/net` на ту же версию (`v0.34.0`), зеркало на GitHub доступно. `replace` работает одинаково и при полном доступе к сети, поведение на CI не меняется. Проверено юнит-тестом на многоуровневом суффиксе `sub.example.co.uk` (написан, прогнан, удалён, не закоммичен). README переписан по правкам: убран абзац про происхождение из bash, убраны бейджи (Go/license/CI), убран раздел «Статус портирования», описание зависимости поправлено (не «копия», а настоящая зависимость `go.mod`).
