...
**Версия: v1.1.8** (2026-08-29)

Changelog (новое сверху, один пункт на одну пушнутую единицу работы):

- **v1.1.8**: политика изменена (раздел 9): баги оригинала теперь чинятся всегда, без исключений на "как в оригинале" — см. раздел 9. Первое применение новой политики: `internal/certs` — `FixLetsencryptStructure` (`check.go`) и `fixRenewHook` (`handle.go`) писали два разных текста `renew_hook` в `renewal.conf` для одной и той же цели (один с завершающим `docker compose exec remnawave-nginx nginx -s reload`, другой без), баг апстрима, воспроизведённый 1:1 в v1.1.7. Унифицировано в общую константу `renewHookCommand` (более полный вариант, с reload-шагом).
...
- **Начиная с v1.1.8: баги оригинала чинятся всегда, без исключений, независимо от размера или давности.** Раньше это правило допускало два варианта ("воспроизводить баг или чинить"); практика (v1.1.5→v1.1.7 с `CADDY_SOCKET_PATH`, v1.1.7→v1.1.8 с `renew_hook`) показала, что решение "оставить как в оригинале" каждый раз приходится пересматривать отдельным пушем позже. Оригинал (`eGamesAPI/remnawave-reverse-proxy`) остаётся источником фактов о том, *что* должен делать непортированный ещё функционал (раздел 8 по-прежнему в силе для этого), но перестаёт быть источником истины о *поведении при найденном баге*: совпадение с оригиналом само по себе больше не аргумент за сохранение дефекта. Найден баг — чинится в той же единице работы, где найден, с комментарием `BUG FIX (not a 1:1 port)` и объяснением причины.
