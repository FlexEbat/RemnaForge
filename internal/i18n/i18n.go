...
var english = map[string]string{
	...
	"CADDY_PROXY_PROTOCOL_WARNING":        "Known issue: this option's Caddyfile needs a proxy_protocol listener module (github.com/mastercactapus/caddy2-proxyprotocol) that is not included in the stock caddy Docker image used here. Caddy will fail to start until the image is rebuilt with that module (xcaddy) or an equivalent fix is applied. See TECH.md.",
...
var russian = map[string]string{
	...
	"CADDY_PROXY_PROTOCOL_WARNING":            "Известная проблема: Caddyfile этого варианта установки требует модуль proxy_protocol (github.com/mastercactapus/caddy2-proxyprotocol), которого нет в стоковом Docker-образе caddy, используемом здесь. Caddy не запустится, пока образ не пересобран с этим модулем (xcaddy) или не применён другой фикс. Подробности в TECH.md.",
