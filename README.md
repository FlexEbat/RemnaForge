...
- **`ExtractDomain` возвращал `co.uk` вместо `example.co.uk`** для домена `sub.example.co.uk`. Оригинальный `awk`-однострочник берёт последние два лейбла домена и не различает составные TLD (`co.uk`, `com.au`, `co.jp`). Go-версия использует `golang.org/x/net/publicsuffix` (вендор в `internal/publicsuffix`, зависимостей кроме stdlib нет) и вызывает `EffectiveTLDPlusOne()`.
- **Сообщения `CONFIG_NOT_FOUND` и `NGINX_CONF_NOT_FOUND` обрывались на «not found in »** без пути. Оригинал ссылается на `$dir` внутри двойных кавычек bash-массива; bash разворачивает переменную в момент загрузки словаря переводов, когда `$dir` ещё не определена, и подставляет пустую строку. Обе строки теперь принимают `%s`, вызывающий код передаёт реальный путь.
- **Установка не проверяла `docker` и `certbot` перед началом.** Отсутствие Docker обнаруживалось через 5 минут повторных попыток как таймаут «containers not ready» без объяснения причины. `internal/preflight` проверяет и, если нужно, ставит зависимости до первого вопроса пользователю.
- **Порядок вывода в «Update current certificates» менялся между запусками.** Итерация по Go `map` не гарантирует порядок обхода, в отличие от bash-массивов, которые сохраняют порядок вставки. Домены теперь сортируются перед обработкой и перед выводом.
- **Парсинг `nginx.conf` в `managepanel` ломался на нестандартном форматировании.** Поиск границ `server {}`-блока опирался на литеральные строки `"server {"` и перевод строки перед `}`. Табы, лишние пробелы или отступ у закрывающей скобки приводили к тому, что парсер не находил блок или находил чужой. `internal/managepanel/nginxconf.go` считает глубину вложенности скобок и находит границы блока независимо от форматирования.
- **Заголовки двух подменю в оригинале ссылались не на те строки локализации.** `show_panel_access()` печатал `LANG[MENU_9]` («Manage certificates domain») вместо описания доступа к панели. `show_manage_certificates()` печатал `LANG[MENU_8]` («Manage IPv6») вместо описания сертификатов. Оба подменю в Go-версии используют правильные ключи.
- **Три сообщения содержали незаполненный `%s`.** `CREATE_NEW_NODE`, `GENERATING_CERTS` и `CHANGE_DIR_FAILED` определены со своим `%s`-плейсхолдером, но в местах вызова значение подставлялось рядом со строкой вместо `fmt.Sprintf`. Пользователь видел буквальный `%s` вместо домена или пути. Все три места вызова исправлены.
- **Сообщение об ошибке главного меню называло диапазон «0-11».** `LANG[INVALID_CHOICE]` использует один текст в двух разных меню: главном (10 пунктов после удаления Custom extensions by legiz) и меню шаблонов selfsteal (3 пункта). Ни один фиксированный диапазон не подходит обоим сразу. Убрал диапазон из текста. Главное меню и так называет актуальный диапазон через `mainMenuPrompt()`, который считает его от реального числа пунктов.
- **Подменю установки называло диапазон «0-5» при 4 реальных пунктах.** `show_install_menu()` выводит 4 варианта (панель с нодой, только панель, добавить ноду, только нода) плюс 0 для выхода. Верный диапазон: «0-4». `INSTALL_PROMPT` и `INSTALL_INVALID_CHOICE` в оригинале говорили «0-5». Исправлено на «0-4» в обоих языках.

## Ограничения

- Caddy и WARP не реализованы. Это выбор объёма работы, не забытая задача.
- Готовых бинарников и установщика одной командой нет. Только сборка из исходников.
- Скрипт требует тщательной проверки перед использованием в проде. Тесты покрывают чистую логику (парсинг доменов, генерацию паролей, рендеринг шаблонов, границы nginx-блоков) и навигацию по меню с поддельными серверами вместо настоящих. `docker compose up`, `certbot certonly` с реальным доменом и `ufw` не тестировались вживую, потому что среда разработки не даёт доступа к Docker и публичному домену. Код собирается и проходит `go vet`. Первый запуск делайте на тестовом сервере, читайте вывод каждого шага и проверяйте результат вручную, прежде чем повторять установку на боевом сервере.
