...
```
cmd/remnawave/          точка входа: проверки ОС и root, затем главное меню
internal/
  ...
  └── ui/                  цвета терминала, чтение ввода
```
...
- Ни один install-флоу не запускался на реальном сервере. Тесты покрывают чистую логику (парсинг доменов, генерацию паролей, рендеринг шаблонов, границы nginx-блоков) и навигацию по меню с поддельными серверами вместо настоящих. `docker compose up`, `certbot certonly` с реальным доменом и `ufw` не тестировались вживую, потому что среда разработки не даёт доступа к Docker и публичному домену. Код собирается и проходит `go vet`. Первый запуск стоит делать на тестовом сервере.
