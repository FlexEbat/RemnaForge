...
- Каждая нода (co-located или отдельная) поднимает три inbound'а на конфиг-профиле панели: VLESS+Reality (`Raw`), Hysteria2 (`HYSTERIA-BBR`, TLS терминирует сам Xray), VLESS+XHTTP (`XHTTP-TLS` поверх `/api/v2/stream-events`). Соответствующие location/route в nginx.conf или Caddyfile и монтирование сертификата ноды в контейнер добавляются автоматически.
